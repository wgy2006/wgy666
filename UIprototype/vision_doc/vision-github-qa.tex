...
\begin{ieeetable}
    \begin{tabularx}{\textwidth}{L{4cm} X}
        ...
        \midrule
        快速获取仓库全景信息 & GitHub 仓库一键同步 + 仪表盘展示 \\
        \midrule
        了解仓库文件构成 & 文件自动分类（10 类）+ 分类统计 \\
        \midrule
        自动识别 Issue 类型 & Issue 规则分类（9 类）+ 置信度评分 \\
        \midrule
        获得 Issue 处理建议 & 分类结果附带建议操作文本 \\
        \midrule
        实时获取新 Issue 通知 & Webhook 监听 + 自动分类入库 \\
        \bottomrule
    \end{tabularx}
\end{ieeetable}
...
\begin{itemize}
    \item \textbf{文件分类}：FileClassifier 按路径规则判断文件类别，共 10 类
    \item \textbf{知识图谱技术}：待评估 Neo4j 或自建图结构
    \item \textbf{向量数据库}：pgvector（与 PostgreSQL 统一）或 Chroma
    \item \textbf{代码分析粒度}：文件级分类，可细化到函数/类级
\end{itemize}
...
\begin{ieeetable}
    \begin{tabularx}{\textwidth}{L{3.5cm} X}
        ...
        版本管理 & Git + GitHub \\
        \midrule
        RAG 框架 & LlamaIndex 或轻量自研 \\
        \midrule
        Agent 框架 & Openai SDK 或自研任务流 \\
        \bottomrule
    \end{tabularx}
\end{ieeetable}
